"""
Nationwide New Balance Store Generator & Clean ETL Pipeline (China Mainland Only - 31 Provincial Regions).
Generates ~1,100+ high-fidelity, standardized, RFC-4180 compliant stores across 260+ cities.
...
def make_store(
    ...
    return {
        ...
        "business_hours": get_business_hours(mall),
        "latitude": str(round(lat + lat_offset, 4)),
        "longitude": str(round(lng + lng_offset, 4)),
...
def build_all_mainland_stores() -> List[Dict]:
    stores = []

    # 1. 北京市 (010, 39.9042, 116.4074)
    beijing_malls = [
    ...
    for m in beijing_malls:
        stores.append(make_store("New Balance", m[0], "北京市", "北京市", m[1], m[2], m[3], m[4], "010", 39.9042, 116.4074, m[5]))

    # 2. 上海市 (021, 31.2304, 121.4737)
    shanghai_malls = [
    ...
    for m in shanghai_malls:
        stores.append(make_store("New Balance", m[0], "上海市", "上海市", m[1], m[2], m[3], m[4], "021", 31.2304, 121.4737, m[5]))

    # 3. 天津市 (022, 39.0842, 117.2009)
    tianjin_malls = [
    ...
    for m in tianjin_malls:
        stores.append(make_store("New Balance", m[0], "天津市", "天津市", m[1], m[2], m[3], m[4], "022", 39.0842, 117.2009, m[5]))

    # 4. 重庆市 (023, 29.5630, 106.5516)
    chongqing_malls = [
    ...
    for m in chongqing_malls:
        stores.append(make_store("New Balance", m[0], "重庆市", "重庆市", m[1], m[2], m[3], m[4], "023", 29.5630, 106.5516, m[5]))
